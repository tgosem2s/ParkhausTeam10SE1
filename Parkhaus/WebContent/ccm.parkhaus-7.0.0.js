...
( function () {
  ...
  var component  = {
    ...
    config: {
      name: "Team 10 Parkhaus",
      server_url: "http://localhost:8080/Parkhaus/Demo",
      Max: 50, // maximum number of parking slots
      open_from: 0,
      open_to: 24,
      delay: 100,
      simulation_speed: 10,
      simulation: {
        ...
        enter: 2,  // enter car into garage for all random choices below this
					// enter value
        ...
        main: {
          inner: [
        	  { tag: 'h2', inner: [
                  ...
                  'Max: &nbsp; ',
                { tag: 'input', class: 'Max', type: 'number', min: 0, value: 50, oninput: '%oninput%' },
                {tag: 'br'},
                {tag: 'hr', style: 'height: 3px; background: black'},
                'Öffnungszeiten von : &nbsp; ',
                { tag: 'input', class: 'open_from', type: 'number', min: 0, max: 24, value: 0, oninput: '%oninput%' },
                ' &nbsp; bis: &nbsp; ',
...
//            { class: 'button_container', inner: [ { tag: 'span', class: 'extra_charts' } ] },
            ...
            { tag: 'h2', style:'text-align: center', inner: [
            	
            	  'Autos im Team 10 Parkhaus:&nbsp;',
            ...
            { class: 'garage', style: 'max-width: 50rem; margin-top: 300px; margin:auto', align: 'centre' },
            {tag: 'hr', style: 'height: 3px; background: black'},
            { tag: 'table', inner: [
...
// { class: 'exit_car_container', inner: { tag: 'img', class: 'exit', src:
// '%empty%', width: '202', height: '74' } },
          ]
        },
      ...
      extra_buttons: [
        ...
        },
        {
            extra_class: 'avg',
            extra_inner: 'Avg',
            extra_popup_title: 'Avg of all parking fees'
          },
          {
              extra_class: 'left',
              extra_inner: 'Left',
              extra_popup_title: 'Total amount of cars that left'
            },
      ],
...
//      ],
      ...
      parking_space: {
        tag: 'svg', id: '%id%', class: 'space', viewBox: '0 0 500 500', xmlns: 'http://www.w3.org/2000/svg', inner: [
          ...
          // "width": "300",
          // "height": "38.11",
          // "style": "fill: rgb(245, 245, 245); stroke-width: 10px; stroke:
			// rgb(0, 0, 0);",
          // "transform": "matrix(0.70711, 0.70711, -0.70711, 0.70711,
			// 274.20139, -90.07433)"
          // },
          // {
          // "tag": "rect",
          ...
          // },
          {
            ...
            "x": "250",
            "y": "350",
            "style": "white-space: pre; fill: rgb(13,70,138); font-size: 200px; text-align: center;",
            "inner": "%nr%"
          }
        ]
      },
      car: {
        tag: 'svg', id: '%id%', class: 'car', style:'width:5rem', viewBox: '0 0 310 255', xmlns: 'http://www.w3.org/2000/svg', inner: [
          {
            "tag": "g",
          ...
          // "tag": "rect",
          // "class": "leave_button",
          // "onclick": "%leave%", // does not work because of SVG Hack
			// innerHTML += '' and re-rendering
          // "style":
			// "stroke:#fff;fill:#fff;fill-opacity:0;stroke-opacity:0;cursor:pointer;",
          // "x": "90",
          // "y": "120",
          // "width": "140",
          // "height": "70"
      ...
      // css: [ 'ccm.load',
		// 'https://ccmjs.github.io/mkaul-components/parkhaus/resources/default.css'
		// ]
        ...
        class Garage {  // single source of truth for Max and all counters
          ...
          driveCarIntoSpace(car ){  // synchronous car moving
            ...
            if ( car && car.space <= this.max ){
              ...
              //if ( (oldChild && oldChild.classList.toString().trim() === 'space') ){  // space
																					// is
																					// free
                (async ()=>{  // only traffic lights are concurrent
                  ...
                  // main_elem.querySelector("img.entry").src =
					// self.images.empty;
                  await sleep( self.delay );
                ...
                // in concurrent car replacements the oldChild might have been
				// replaced in the meanwhile
...
// exit_car_container.replaceChild( leavingCar,
// exit_car_container.firstElementChild );
// leavingCar.innerHTML += '';
// setTimeout(
// () => {
// exit_car_container.replaceChild( exit_img,
// exit_car_container.firstElementChild );
// },
// 5 * self.delay
// );
...
//           var test = Object.keys(this._space_index).length;
//           for (var i=1; i<11; i++){
//        	   if(('Space_' + i) in this._space_index){
//        		   test -=1;
//        	   }
//          }
        	//  while(test<(this._max - this._handip)){
        	  while(true){
        		 console.log(this._space_index);
        	var temp =  1 + Math.floor(Math.random()*(this.max));
            var temp2 = 'Space_' + temp;
            
        	  if(  temp2 in this._space_index === false){
...
//          randomSpaceHandi(){ // start with 1, end with max
//              while(true){
//           	var temp = 1 + Math.floor(Math.random()*this.max);
//               var temp2 = 'Space_' + temp;
//               
...
//             }
          ...
          rerender(){
            garage_div.innerHTML += ''; // SVG Hack: force re-rendering; Thereby
        ...
        class Car {
        	...
        	constructor( spec ){        		
              ...
              this.nr = garage.nextTotal();  // because the car has not
												// entered the car_index
          ...
          get space(){
              if ( ! this._space ) this._space = this.temp;
              return this._space;
            }
          set space( nr ){   	  
        	  if ( nr ){
          ...
          price() {
            ...
            // return this._duration ? ' € ' +
			// parseFloat(Math.round(this._duration / self.simulation_speed ) /
			// 100).toFixed(2) : null;
          }
          color(){
            if ( ! this._color ){
        ...
        function command_interpreter( response_string, extra_span ){
          ...
          } else { // generic interpreter for server responses: Where? What?
					// Params...
